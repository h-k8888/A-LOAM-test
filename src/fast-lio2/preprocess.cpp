...
//      pcl::PointCloud<PointType_XYZIN> &pl = pl_buff[j];//当前line的点云
//      plsize = pl.size();
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    //todo Failed to find match for field 'intensity'.
    pcl::fromROSMsg(*msg, pl_orig);
...
//    int cloudSize = pl_orig.points.size();
//    float startOri = -atan2(pl_orig.points[0].y, pl_orig.points[0].x);
//    float endOri = -atan2(pl_orig.points[cloudSize - 1].y,
//                          pl_orig.points[cloudSize - 1].x) +
//                   2 * M_PI;
//
...
//    int count = cloudSize;
//    PointType_XYZIN point;
//    std::vector<pcl::PointCloud<PointType_XYZIN>> laserCloudScans(N_SCANS);
...
//        point.curvature = scanID + scanPeriod * relTime;
//        pl_orig.points[i].time = point.curvature;
////        ROS_INFO("pl_orig.points[%d].time %f", i,  pl_orig.points[i].time);
...
//    }
    ...
    if(feature_enabled)
    {
      ...
      float time_min = static_cast<float> (INT_MAX);//unit: ms
      float time_max = static_cast<float> (INT_MIN);//unit: ms
...
//        ROS_INFO("time_min: %f  max %f", time_min, time_max);
...
void Preprocess::give_feature(pcl::PointCloud<PointType_XYZIN> &pl, vector<orgtype> &types)
{
  int plsize = pl.size();
